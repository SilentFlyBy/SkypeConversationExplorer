        ...
        public IEnumerable<Messages> GetByAccountName(string accountname, DateTime start, DateTime end)
        {
            if (start != null && end != null)
            {
                return (from Messages messages in entities.Messages
                        where messages.author == accountname && Utils.TimestampToDateTime(messages.timestamp.Value) > start && Utils.TimestampToDateTime(messages.timestamp.Value) < end
                        select messages).ToArray().ToList<Messages>();
            }
            else if (start != null)
            {
                return (from Messages messages in entities.Messages
                        where messages.author == accountname && Utils.TimestampToDateTime(messages.timestamp.Value) > start
                        select messages).ToArray().ToList<Messages>();
            }
            else if (end != null)
            {
                return (from Messages messages in entities.Messages
                        where messages.author == accountname && Utils.TimestampToDateTime(messages.timestamp.Value) < end
                        select messages).ToArray().ToList<Messages>();
            }
            else
            {
                return (from Messages messages in entities.Messages
                        where messages.author == accountname
                        select messages).ToArray().ToList<Messages>();
            }
        }
    }
